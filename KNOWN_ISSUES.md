...
**New structural bug found during the fix pass, NOT fixed (out of scope for this explanation-focused task) — same class as Gujarati's missing-passage bug (`e0734b0`):** `FarsiFunctionalSkillsExerciseData.swift` has **29 items across 3 exercise types** whose prompts explicitly reference a document to read ("زیر را بخوانید" / "read below") but whose `passage` field is null — the document was never populated, making these items unanswerable as written, not just low-quality. Breakdown: 17 in "Reading Menus" (idx 136033–141949), 8 in "Reading Prescriptions" (idx 142702–148270, medication labels/dosage instructions), 4 in "زمان و ریاضی" (idx 124015–128195, utility bill amounts). Recommended fix: write 29 new Farsi-script passages (menus, medication labels, utility bills) grounded in the established Iranian-American community context, following the exact same method used for Gujarati's `e0734b0` fix (including recomputing/verifying any math-based items' arithmetic against the passage content). Note: a separate, smaller pre-existing math/logic concern was also checked during this pass — the time/math explanation-regeneration agent independently re-verified arithmetic for all 74 items in that section and found zero discrepancies, so this is isolated to the missing-passage structural bug only, not a wider math-correctness problem.
