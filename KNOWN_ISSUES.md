...
**Farsi — needs real fixes, one severe file (FunctionalSkills).**
- All 15 `FarsiCognitionExerciseData.swift` duplicate prompts traced to root cause: 3 categories (History, Locations, Estimation/Logic) were generated by copy-pasting items verbatim out of 3 earlier WH-question batches (Where/Who/When/HowMany) rather than writing new trivia. Full list of all 15 pairs with line numbers exists in the agent's original report.
- CJK contamination found inside an actual **graded `correctAnswer` field**, not just an explanation — `FarsiHardExercisesData.swift:179`, a plastic-bag-ban cause/effect item's correct answer contains Chinese "环保" instead of the intended Persian word for "reusable."
- Arabic-vs-Persian letter-form mixing (Arabic ي/ك used instead of Persian ی/ک) in a section title (`FarsiHardExercisesData.swift:354`) and 2 other items — a real Persian-orthography quality bug, not just script contamination from an unrelated language.
- 159 additional English-only explanations beyond the 77 templated ones, concentrated in 4 entire functional sub-domains (time-telling, budgeting/shopping, restaurant menus, medication labels) that appear never to have been localized at all.
- One factual/category-logic error: `FarsiLanguageExerciseData.swift:311` labels a false historical claim (Persepolis built by Cyrus, actually Darius I) as "Opinion" instead of a factual error — teaches the wrong reasoning skill.
